...
export default function Socios() {
  ...
  const handleDelete = (email: string) => {
    const ok = window.confirm('¿Eliminar socio? Esta acción no se puede deshacer.')
    if (!ok) return
            ...
            <TableWrapper>
              <StyledTable>
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={5}><MutedText>Cargando…</MutedText></td>
                    </tr>
                  ) : list.length === 0 ? (
                    <tr>
                      <td colSpan={5}><MutedText>Sin resultados</MutedText></td>
                    </tr>
                  ) : (
                    list.map((s) => (
                      <tr key={s.email}>
                        <td><PrimaryText>{s.nombre}</PrimaryText></td>
                        <td><MutedText>{s.email}</MutedText></td>
                        <td><MutedText>{s.sector}</MutedText></td>
                        <td><MutedText>{Array.isArray(s.intereses) ? s.intereses.join(', ') : String(s.intereses)}</MutedText></td>
                        <td>
                          <RowActions>
                            <DropdownMenu.Root>
