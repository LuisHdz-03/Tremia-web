...
const USERS_SOCIO_PATH: string = (import.meta as any)?.env?.VITE_USERS_SOCIO_PATH ?? '/usuarios/rol/socio'
...
export async function getSociosUsuariosApi(): Promise<SocioUsuario[]> {
  try {
    ...
    console.error('getSociosUsuariosApi error', { status, respData, url: USERS_SOCIO_PATH })
    throw err
  }
}
...
const USERS_ORG_PATH: string = (import.meta as any)?.env?.VITE_USERS_ORG_PATH ?? '/usuarios/rol/organizacion'

// GET /usuarios/rol/Organizacion
export async function getOrganizacionUsuariosApi(): Promise<OrganizacionUsuario[]> {
  try {
    ...
    const respData = err?.response?.data
    // eslint-disable-next-line no-console
    console.error('getOrganizacionUsuariosApi error', { status, respData, url: USERS_ORG_PATH })
    throw err
  }
}
